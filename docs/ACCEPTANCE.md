...
- Versioned atomic JSON persistence, schema-0/1 migration, scene-wide appearance plus per-widget opt-out, unknown-setting/type preservation, and legacy data migration.
...
- The current-source portable V0.3 Release smoke passed with all production widgets present. It verified one guarded process, registry enumeration, all four production widget types, multiple instances, matching per-widget HWND counts, passive `HTTRANSPARENT` margins, Widget Studio preview-above-settings/no-scroll layout, general font propagation to participating widgets (including Calendar), per-widget appearance opt-out and font override, safe Grid-to-Free conversion, a Clock-specific setting, state-preserving duplication, removal, generic Lock All, normal Exit, and schema-2 scene/HWND restoration after relaunch. Explorer exposed no usable WorkerW in this session, so the smoke also verified the safe top-level fallback.
...
- Startup-path review found that the only deliberate pre-loop success return was the single-instance guard. A diagnostic launch reproduced the reported short-lived-process behavior while another WidgetStudio copy owned the mutex; the new build returned code `2` and recorded the existing PID and executable path. Startup now logs every core stage and converts initialization-time host destruction, `GetMessage` failure, and all subsystem failures to non-zero exits.
- The stabilization audit removed the first-run concrete type ID from `DesktopHost`, bounded and de-overlapped free-layout duplication, added defensive non-finite restore sanitization, canceled both desktop and Studio-preview capture on mode exit, corrected render-target resize recovery, surfaced persistent render/library/Explorer-tray failures in the local diagnostic log, and added deterministic lifecycle/layout regression tests.
- Current MSVC Debug and Release builds are warning-clean. The current Release logic suite passed 1/1 from `C:\WidgetStudioBuild\startup-diagnostic\Release`; local Application Control still blocks newly emitted unsigned test executables in the ordinary `C:\WidgetStudioBuild\Debug` and `Release` roots before process start. GitHub-hosted Debug/Release CTest remains the authoritative clean-policy gate for the next pushed commit.
...
- On the Windows 10 portable test machine, create at least three DebugWidgets, verify first-free placement, single/Shift selection and primary styling, drag capture/release outside a widget, locked selection without movement, bounded Ctrl+D duplication, multi-delete, Esc from both desktop and Studio-preview dragging, tray-menu dismissal, normal Exit/relaunch, and `data\config\scene.json` restoration. Rename that file temporarily to validate clean first-run recovery, then restore it; do not overwrite the malformed/original file during the check.
