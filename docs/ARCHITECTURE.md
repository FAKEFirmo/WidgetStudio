...
`WidgetStudioWindow` renders the same `WidgetScene` through the same `Renderer`, `GridLayout`, and authored widget content used by the desktop. Its preview uses the full settings-workspace width, preserves the selected monitor aspect ratio, and uses the same monitor backing bitmap as the desktop widget HWNDs; the settings surface scrolls vertically instead of shrinking the preview into side gutters. Scrolling repositions the complete native child-window set atomically with `DeferWindowPos` and discards copied client pixels. Repainting is deliberately two-phase: the solid parent surface first erases all space outside the controls at their new positions, including every former control location, and only then are visible children and their non-client frames redrawn. Dark owner-drawn Layout, Appearance, Widget content, and Actions navigation keeps related controls together without relying on the light-themed Win32 tab surface. A preview-only render transform fits the active monitor scene without maintaining duplicate layout state. Universal instance fields and declarative `IWidget::Settings()` definitions drive native controls for numbers, booleans, choices, text, and local files. Every widget-specific definition is shown as a direct auto-saving control; persisted choice values remain separate from their user-facing labels. Universal appearance includes theme, frosted/transparent/solid surface mode, opacity, blur, radius, padding, border, and shadow. Changes apply to the live scene and are persisted through the normal scene-change callback. A topology-backed monitor selector moves selected instances generically and switches the preview to the destination monitor.
