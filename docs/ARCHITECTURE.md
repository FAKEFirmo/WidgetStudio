...
- Configure: general appearance lives on `WidgetScene`; opted-in instances follow it, while opted-out instances retain their own appearance. Widget-specific content settings are described by `IWidget::Settings()`.
- Serialize: the scene produces a snapshot containing general appearance plus records with stable IDs, both layout modes, per-widget appearance participation/overrides, locking, scale, and widget state. `SceneJsonCodec` writes the versioned schema.
...
`WidgetSceneSnapshot` is the encoding-neutral persistence model. It contains the scene-wide general appearance and widget records. Each record contains instance/type/monitor IDs, grid and free placements, layout mode, lock state, content scale, effective appearance, a general-appearance participation flag, and widget-specific key/value state.

`SceneJsonCodec` encodes schema version 2 without a third-party dependency and rejects malformed input, unsupported future versions, invalid values, and duplicate instance IDs. Schema versions 0 and 1 migrate without a visual change: the first existing appearance becomes the general default, matching widgets participate in it, and differently customized widgets remain opted out. Schema version 0 also migrates missing monitor associations to the primary display. Unknown JSON fields are ignored safely. The scene preserves unknown widget-state keys when a known older widget restores and re-saves a newer record. Records whose widget type is unavailable are retained and written back instead of being discarded.
...
`RenderingResources` owns one process-wide Direct2D factory, shared DirectWrite factory, WIC factory, and common text formats. Per-widget `Renderer` objects retain only their HWND-specific software render targets and device-dependent bitmap caches. Software targets avoid driver-dependent composition artifacts in the transparent desktop windows and management preview while preserving the event-driven rendering model. This preserves independent lightweight windows while avoiding a full graphics-factory stack for every widget.
...
`WidgetStudioWindow` renders the same `WidgetScene` through the same `Renderer`, `GridLayout`, and authored widget content used by the desktop. Its preview preserves the selected monitor aspect ratio and uses the same monitor backing bitmap as the desktop widget HWNDs. The preview scales to the space remaining above the active settings category, so the minimum supported window size exposes every setting without a scrolling surface. Layout uses the otherwise empty cell in its final two-column row for the apply action. Dark owner-drawn General, Layout, Widget style, Widget content, and Actions navigation keeps related controls together without relying on the light-themed Win32 tab surface. A preview-only render transform fits the active monitor scene without maintaining duplicate layout state. The General page owns the scene-wide font, tint, theme, surface, opacity, blur, radius, padding, border, and shadow defaults and updates every participating widget immediately. Widget style exposes an explicit `Use general settings` checkbox; disabling it unlocks the same fields as per-widget overrides. The chosen font flows through the shared render context, so Clock, Calendar, Music, Photo placeholders, and diagnostic text all use the same appearance boundary. Declarative `IWidget::Settings()` definitions remain reserved for content-specific numbers, booleans, choices, text, and local files. Every widget-specific definition is shown as a direct auto-saving control; persisted choice values remain separate from their user-facing labels. Changes apply to the live scene and are persisted through the normal scene-change callback. A topology-backed monitor selector moves selected instances generically and switches the preview to the destination monitor.
...
`MonitorTopology` enumerates stable display device IDs, effective DPI, full-monitor pixels/DIPs, monitor-local work-area bounds, and virtual-desktop pixels. The scene and its 12 x 7 grid always use the complete `rcMonitor`; the work area is retained separately for policies that intentionally avoid reserved shell space. Cells remain square and use the configured 10 DIP minimum gap. The limiting axis keeps that gap, while the non-limiting axis distributes its aspect-ratio remainder across that axis's gaps so both outer edges reach the configured 18 DIP margin. Horizontal and vertical strides remain shared metrics for deterministic aligned widget edges. The last interacted widget window becomes the Widget Studio and Widget Library target. Display changes reposition all widget HWNDs; every instance is reconciled with the full monitor, while instances whose saved display disappeared first migrate to the primary monitor. Any corrected geometry is saved. Per-widget `WM_DPICHANGED` notifications coalesce into one topology refresh. After Explorer broadcasts `TaskbarCreated`, surviving windows reattach and destroyed child windows are recreated from the scene.

Coordinate spaces are explicit at the native boundary: scene grid/free placement and drag offsets are full-monitor-local DIPs; wallpaper backing surfaces and source rectangles are physical pixels. `WidgetWindowPlacementCalculator` is the only scene-to-screen placement conversion and adds the full-monitor physical origin after scaling DIPs exactly once by the monitor DPI. Mouse messages begin as client pixels, are converted with the widget HWND DPI, then add the monitor-local window origin before entering layout/drag code. Direct2D targets consume DIPs, while wallpaper crops remain in physical pixels until their final one-to-one target mapping. Debug Studio previews show monitor size, effective DPI, grid dimensions, selected widget DIPs, and wallpaper source pixels for direct verification.
