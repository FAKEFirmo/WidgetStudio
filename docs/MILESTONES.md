...
Persistence now uses schema version 2 for scene-wide general appearance and per-widget participation, migrates schema versions 0 and 1 without changing their existing widget appearance (matching old appearances join the general group; customized ones opt out), preserves unknown future widget-state keys, and copies legacy scene/photo data into the current directory hierarchy without deleting the originals. Widget Studio provides fixed no-scroll General and Widget style pages; global font/tint/surface settings propagate to participating widgets, while individual widgets can opt out and retain overrides. Font selection now reaches Calendar and every other widget through the shared render context. Music transport rendering uses paired triangle geometry and rounded pause bars rather than font glyphs.

The current per-widget-window milestone compiles and links warning-clean with MSVC 19.51.36256. Clean CMake/Ninja Debug and Release configure/builds succeeded under `C:\WidgetStudioBuild`, both current CTests passed 1/1, and V0.3 package creation succeeded. The current-source Release UI smoke exercised every production widget type, registry-driven creation, multiple instances, passive margins, the fixed no-scroll settings pages, general font propagation (including Calendar), per-widget appearance opt-out/font override, safe Grid/Free conversion, widget-specific settings, state-preserving duplicate/remove, Lock All, single-instance enforcement, and schema-2 restart restoration. Explorer exposed no usable WorkerW in that session, so the safe top-level fallback was exercised.
