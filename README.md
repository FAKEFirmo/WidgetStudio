...
WidgetStudio alpha builds use `V0.<major>[.<patch>]`. The baseline is `V0.0`; major alpha improvements increment the second number, while bug fixes, tweaks, and small improvements add or increment the third number. The current build is `V0.3`.
...
- A compact dark settings workspace with shared general appearance defaults, per-widget opt-out and font/tint overrides, direct auto-saving content controls, monitor assignment, and a shared-scene preview.
